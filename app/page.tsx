      ...
      <main className="max-w-2xl mx-auto px-6 py-20">
        ...
        {step === "processing" && (
          <div className="rounded-2xl border border-zinc-100 bg-zinc-50 p-6 space-y-4 mb-8">
            <p className="text-xs font-medium text-zinc-500 uppercase tracking-widest">진행 상황</p>
            {[
              { label: "음성 전사 (STT)", done: true },
              { label: "텍스트 번역", done: false },
              { label: "음성 합성 (TTS)", done: false },
            ].map((item, i) => (
              <div key={i} className="flex items-center gap-3">
                <div className={[
                  "w-4 h-4 rounded-full flex items-center justify-center flex-shrink-0",
                  item.done ? "bg-zinc-900" : "border-2 border-zinc-300",
                ].join(" ")}>
                  {item.done && (
                ...
                <span className={item.done ? "text-sm text-zinc-900 font-medium" : "text-sm text-zinc-400"}>
                  {item.label}
                </span>
              </div>
            ))}
          ...
          <div className="space-y-6">
            <div className="rounded-2xl border border-zinc-200 p-6">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <p className="text-sm font-semibold text-zinc-900">더빙 완료</p>
                  <p className="text-xs text-zinc-400 mt-0.5">
                    {LANGUAGES.find(l => l.code === targetLang)?.label} 버전이 준비되었어요
                  </p>
                </div>
              </div>
              ...
              <div className="bg-zinc-50 rounded-xl p-4 mb-4">
                <p className="text-xs text-zinc-400 mb-3">미리 듣기</p>
                <div className="flex items-center gap-3">
                  <button className="w-9 h-9 rounded-full bg-zinc-900 flex items-center justify-center hover:bg-zinc-700 transition-colors flex-shrink-0">
                    <svg width="12" height="12" viewBox="0 0 24 24" fill="white">
                      <polygon points="5 3 19 12 5 21 5 3" />
                    </svg>
                  </button>
                  <div className="flex-1 h-1 bg-zinc-200 rounded-full">
                    <div className="w-0 h-full bg-zinc-900 rounded-full" />
                  </div>
                  <span className="text-xs text-zinc-400 font-mono">0:00</span>
                </div>
              </div>

              <button className="w-full py-3 rounded-xl border border-zinc-900 text-sm font-semibold text-zinc-900 hover:bg-zinc-900 hover:text-white transition-all duration-150 flex items-center justify-center gap-2">
